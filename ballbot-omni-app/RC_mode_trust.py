...
#from src.bot_kinematic import compute_robot_motor_torques
...
if __name__ == "__main__":
    ...
    #filename = f"lab9_data_trial_{trial_num}.txt"
    filename = f"lab10_data_trial_{trial_num}.txt"
    dl = dataLogger(filename)
    #pid_files = f"PID_data_trial{trial_num}.txt"
    #pid_dl = dataLogger(pid_files)
    ...
    for t in SoftRealtimeLoop(dt=DT, report=True):
        try:
            ...
            desired_angle = 0.0
            x_val = states['theta_roll']  # Roll angle (x-axis)
            y_val = states['theta_pitch']  # Pitch angle (y-axis)
            z_val = states['theta_yaw'] #Yaw angle (z-axis)

            errorx = desired_angle - x_val
            errory = desired_angle - y_val
            errorz = desired_angle - z_val
            ...
            # T_out_iy = Ki_y * -errory * DT (OLD IMPLEMENTATION VERY BAD)
            T_out_iy = Ki_y * -(sumy + errory) * DT
            # T_out_dy = (Kd_y * -errory)/DT (OLD IMPLEMENTATION VERY BAD)
            ...
            theta_z = states['theta_yaw'] #Yaw angle (z-axis)
            ...
            #data = [i, t_motor_now, theta_x, theta_y, Tx, Ty, Tz, T1, T2, T3, psi_1, psi_2, psi_3]
            #data = [i, t_motor_now, theta_x, theta_y, theta_z, dpsi1, dpsi2, dpsi3]
            data = [i, t_now, theta_x, theta_y, Tx, Ty, Tz, T1, T2, T3, psi_1, psi_2, psi_3]
            # FOR PID DATA LOGGING: 
            # data = [i, t_now, theta_x, theta_y, Tx, Ty, Tz, T1, T2, T3, psi_1, psi_2, psi_3, dpsi_1, dpsi_2, dpsi_3] ?
            #data2= [i, t_now, x_val, y_val, z_val, T_out_dx, T_out_ix, T_out_px] 
            dl.appendData(data)
            #d1.appendData(data2)

            # Print debug information
            #print(
            #    f"Time: {t_motor_now:.2f}s | Tx: {Tx:.2f}, Ty: {Ty:.2f}, Tz: {Tz:.2f} | "
            ...
            print(
                ...
                f"T1: {T1:.2f}, T2: {T2:.2f}, T3: {T3:.2f} | "
                f"Roll: {theta_x:.2f}, Pitch: {theta_y:.2f}, Yaw: {theta_z:.2f}"
            )
